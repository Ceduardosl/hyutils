...
def get_Curve_HU_value(t, HU, Dim_HU, name):
...
def Conv_HU (pef, HU, d, name):
    ...
    for i in range(HU_intervals):
        HU_q[i] = get_Curve_HU_value(i*d, HU, Dim_HU, name)
...
for i in range(len(info_df)):
    ...
    area = info_df["Area"][i]/100 #para km²
    tc = info_df["tc"][i]
    CN = info_df["CN"][i]
...
#%%
    if name == "Fronteira":
        ...
        isozona = "D"
        pr_tr = tr_df.loc[name,:]
    
        tc = time_of_concetration.USACE(Le, hf, hi)
    
    project_rain = project_hyetograph.Taborga(pr_tr, 
                                    isozona = "D",
                                    coef_Taborga_24h = 1.1,
                                    tc = tc,
                                    d = d
                                    )
